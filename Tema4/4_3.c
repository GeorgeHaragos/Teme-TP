...
int main(void){
  struct Fisier *fisier; // il declaram ca pointer fiindca nu stim dimensiunea exacta a vectorului
  ...
  scanf("%u",&n);
  fisier=(struct Fisier *)malloc(n*sizeof(struct Fisier *)); // alocarea dinamica
  if(!fisier){
      fprintf(stderr,"Eroare alocare memorie dinamic!\n");
      return 1;
  }
  ...
  fprintf(f,"Tipul fisierului: "); //tiparim tipul fisierului
  switch(fisier[i].tip){
